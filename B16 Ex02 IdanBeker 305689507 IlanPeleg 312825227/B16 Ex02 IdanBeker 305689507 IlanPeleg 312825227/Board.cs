...
/////////////////  ZEEVIKS VERSION OF DIAPLAY THE BOARD
        ...
        board.Append(createNumbersRow()).Append(System.Environment.NewLine);
        for (int i = 0; i < m_BoardHeight; i++)
        {
            for (int j = 0; j < m_BoardLength; j++)
            {
                currentPiece = m_Board[i, j].ToString();
                /// If the spot is empty, use whitespace
                printToBoard = currentPiece.EndsWith("Empty") ? " " : currentPiece;
                row = string.Format("| {0} ", printToBoard);
                board.Append(row);
            }

            board.Append("|").Append(System.Environment.NewLine);
            board.Append(createSeparetionRow()).Append(System.Environment.NewLine);
        }
        ...
        */
        ...
        /*
        private string createSeparetionRow()
        {
            StringBuilder separetionRow = new StringBuilder();
            for (int i = 1; i <= m_BoardLength; i++)
            {
                separetionRow.Append("====");
            }

            separetionRow.Append("=");

            return separetionRow.ToString();
        }
        private string createNumbersRow()
        {
            StringBuilder numbersRow = new StringBuilder();
            for (int i = 1; i <= m_BoardLength; i++)
            {
                string columnNumber = string.Format("  {0} ", i);
                numbersRow.Append(columnNumber);
            }
